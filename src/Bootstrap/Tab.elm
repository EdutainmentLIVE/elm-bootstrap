module Bootstrap.Tab exposing
    ( view, view_, config, items, initialState, customInitialState, Config, State
        ...
        Just (Item currentItem) ->
            Html.div []
                [ Html.ul
                    (tabAttributes configRec)
                    (List.map
                        (\(Item item_) -> renderLink_ item_.id (item_.id == currentItem.id) item_.link configRec strToMsg)
                        configRec.items
                    )
                , Html.div
                    [ class "tab-content" ]
